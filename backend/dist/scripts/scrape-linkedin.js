...
const wait_for_enter_1 = require("./wait-for-enter");
...
async function extractProfileFromHTML(element, index) {
...
async function scrollToLoadMoreProfiles(page, targetCount = 20) {
    ...
    while (currentCount < targetCount && scrollAttempts < maxScrollAttempts) {
        ...
        if (scrollAttempts > 3 && currentCount < targetCount) {
            try {
                const seeMoreButton = await page.$('button:has-text("See more results"), button:has-text("Show more"), .artdeco-pagination__button--next');
                if (seeMoreButton) {
...
async function saveToDatabase(profiles) {
    try {
        ...
        if (savedCount > 0) {
            ...
            const recentProfiles = await Profile_1.default.find().sort({ createdAt: -1 }).limit(savedCount);
...
async function scrapeLinkedInProfiles(searchUrl, credentials) {
    ...
    try {
        ...
        await (0, wait_for_enter_1.waitForEnter)("👉 Press ENTER when ready to continue...");
        ...
        for (let i = 0; i < profileElements.length; i++) {
            ...
            // Progress indicator
            if ((i + 1) % 5 === 0) {
                console.log(`   📈 Progress: ${i + 1}/${profileElements.length} processed, ${profiles.length} valid profiles found`);
            }
            await humanLikeDelay(500, 1500);
            if (i % 3 === 0) {
                await simulateHumanBehavior(page);
...
async function main() {
    ...
    console.log("================================================");
    const searchUrl = "https://www.linkedin.com/search/results/people/?geoUrn=%5B%22103644278%22%5D&industry=%5B%221594%22%2C%221862%22%2C%2280%22%5D&keywords=%22lead%20generation%20agency%22&origin=GLOBAL_SEARCH_HEADER";
    await scrapeLinkedInProfiles(searchUrl, { email, password });
}
main().catch(console.error);
//# sourceMappingURL=scrape-linkedin.js.map
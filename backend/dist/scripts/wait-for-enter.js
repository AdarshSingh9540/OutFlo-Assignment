...
async function autoScroll(page, steps = 5, delay = 1000) {
    ...
    for (const selector of possibleSelectors) {
        try {
            ...
            if (elements.length > 0) {
                ...
                console.log(` Sample HTML for ${selector}:`, html.substring(0, 200) + "...");
            }
        }
        catch (error) {
...
async function scrapeLinkedInProfiles(searchUrl, credentials) {
    ...
    try {
        ...
        try {
            ...
            console.log(` Found ${profileElements.length} results using selector: ${workingSelector}`);
        }
        catch (error) {
            console.log(` Could not find profile elements with selector: ${workingSelector}`);
        ...
        for (let i = 0; i < Math.min(profileElements.length, 20); i++) {
            ...
            try {
                ...
                if (name && profileUrl) {
                    ...
                    console.log(`   Successfully extracted: ${name} | ${jobTitle} | ${location}`);
                }
                else {
                    console.log(`   Skipped profile ${i + 1} - Name: "${name}", URL: "${profileUrl}"`);
                }
        ...
        if (profiles.length > 0) {
            ...
            await Profile_1.default.insertMany(profiles);
...
//# sourceMappingURL=wait-for-enter.js.map